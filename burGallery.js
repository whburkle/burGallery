...
--------------------------------- 
	...
	* @classdesc Photo gallery featuring dynamic image and thumbnail builds, default or custom markup, slideshow, auto-play/pause, slideshow speed, keyboard functionality, multiple ways to close gallery, thumbnails, inline or modal box, and much more!
	...
	* 
	* Open viewed images inline (use the ".modal_content" div WITHOUT a ".modal" div as a wrapper) or within a modal (use the ".modal_content" div WITHIN a ".modal" div wrapper).
	* - Use only ".modal_content" div for an inline gallery (manually remove the ".modal" div wrapper from your html page markup).
	* - Use only ".modal" div for a full screen gallery (manually remove the ".modal" div wrapper from your html page markup).
	* 
	* Keyboard functionality with arrows left/right and enter for gallery img navigation, as well as escape to close the album/slideshow, and spacebar to play/pause.
	* Mobile/touch support with swipe capabilities for gallery img navigation.
	* Appearance mode
	* Image Captions
	...
	* 
	* Slideshow funtionality with play/pause, slideshow speed, auto-play, and auto-display, loop, appearance mode, full window, full screen, download image.
	* 
	...
	* Then you must pass a name (gallery name, for example "portfolio"), relative url to your gallery content/images, along with any addional optional "portfolio_options" if you'd like to customize, such as slideshow speed, loop, or templates, etc. 
	...
	* Auto-play slideshow on page load (true/false)
...
class Gallery {
	...
	constructor(name, dbase_file_relative_url, options) {
		...
		this.containers = {
			...
			appearance_btn: undefined, // this.containers.gallery.querySelector(".appearance_btn") // was "toggle_mode_btn" // Toggle appearance mode
			caption_btn: undefined, // this.containers.gallery.querySelector(".caption_btn") // Toggle Caption for image(s)
		...
		// All the main arrays referenced thoughout, initally empty
		...
		if (options.appearance_mode) {
			this.status.appearance_mode = options.appearance_mode;
		}
		
		if (options.caption) {
			this.status.caption = options.caption;
		}

		if (options.custom_event) {
			this.status.custom_event = options.custom_event;
		}

		if (options.display) {
			this.status.display = options.display;
		}

		if (options.loop) {
			this.status.loop = options.loop;
		}

		if (options.play) {
			this.status.play = options.play;
		}

		if (options.slideshow_speed) {
			this.status.slideshow_speed = options.slideshow_speed;
		}
		...
		if (options.template) {
			// Images
			if (options.template.images_wrapper_open) {
				this.template.images_wrapper_open = options.template.images_wrapper_open;
			}

			if (options.template.images_wrapper_content) {
				this.template.images_wrapper_content = options.template.images_wrapper_content;
			}

			if (options.template.images_wrapper_close) {
				this.template.images_wrapper_close = options.template.images_wrapper_close;
			}

			// Thumbnails
			if (options.template.grid_wrapper_open) {
				this.template.grid_wrapper_open = options.template.grid_wrapper_open;
			}

			if (options.template.grid_wrapper_content) {
				this.template.grid_wrapper_content = options.template.grid_wrapper_content;
			}

			if (options.template.grid_wrapper_close) {
				this.template.grid_wrapper_close = options.template.grid_wrapper_close;
			}

			// Scroll
			if (options.template.scroll) {
				this.template.scroll = options.template.scroll;
			}

			// Album navigation
			if (options.template.album_navigation_menu) {
				this.template.album_navigation_menu = options.template.album_navigation_menu;
			}
		}
	...
	load_gallery() {
		...
		xmlhttp.onreadystatechange = (eventData) => {
	...
	build_gallery() {
		...
		// if all is good, move forward with building the gallery
			...
			// Check to see if each component exists in html markup. If so, associate each with a respective object in the gallery class
			this.component_name.forEach((obj) => { /* element, index, array */
				if (this.containers.gallery.querySelector(`.${ obj }`)) { 
				    this.containers[obj] = this.containers.gallery.querySelector(`.${ obj }`);
				}
	   		});
			...
			this.component_array.dbase.forEach((element) => { /* element, index, array */
				...
				if (!element[0].album || element[0].album === "" || 
					...
					if (this.containers.grid) {
						this.markup.grid += `<div class='album_${ this.status.album_num }'>`;
					}

					if (this.containers.scroll) {
						this.markup.scroll += `<div class='album_${ this.status.album_num }'>`;
					}
					...
					if (this.containers.grid) {
						this.markup.grid += "</div>";
					}

					if (this.containers.scroll) {
						this.markup.scroll += "</div>";
					}
				}
	   		...
	   		// If modal is included, set it to display = none by default
			...
			if (this.containers.grid) { 
				this.containers.grid.innerHTML = this.markup.grid;
			}	

			if (this.containers.scroll) { 
				this.containers.scroll.innerHTML = this.markup.scroll;
			}
			...
			if (this.containers.counter) {
				this.containers.counter.innerHTML = "";
			}



			// Create the album navigation menu items
			if (this.containers.grid || this.containers.scroll || this.containers.album_navigation) {
				this.template_check_2(); // Check if custom templates were provided for album navigation
			}
	...
	open_gallery(a, i) { // a: album, i: image
		...
		if (this.containers.grid && this.containers.grid.querySelector(".active")) {
			let prev_grid_thb = this.containers.grid.querySelector(".active");
			prev_grid_thb.classList.remove('active');
			prev_grid_thb.disabled = false;
		}

		// And only one scroll thumbnail is active at a time by deactiving current selection
		if (this.containers.scroll && this.containers.scroll.querySelector(".active")) {
			let prev_scroll_thb = this.containers.scroll.querySelector(".active");
			prev_scroll_thb.classList.remove('active');
			prev_scroll_thb.disabled = false;
		}
		...
		if (!this.status.album_num || this.status.album_num === "") {
			...
			if (this.containers.album_navigation) {
		    	if (this.containers.album_navigation.querySelector(".active")) {
		    		let previous_nav_link = this.containers.album_navigation.querySelector(".active");
		    		previous_nav_link.classList.remove('active');
		    		previous_nav_link.style.pointerEvents = "auto";
		    	}
		    	...
		    	current_nav_link.style.pointerEvents = "none";
		    }
	...
	close_gallery() {
		...
		if (this.containers.grid) {
			this.component_array.grid_thb[this.status.img_index].classList.remove('active');
			this.component_array.grid_thb[this.status.img_index].disabled = false;
		}

		if (this.containers.scroll) {
			this.component_array.scroll_thb[this.status.img_index].classList.remove('active');
			this.component_array.scroll_thb[this.status.img_index].disabled = false;
		}
	...
	/* Method to check current img number and set current image/thumb properties, such as img_index, then call show_current_img
	--------------------------------- */ 
	set_img_status() {
		// Check value of img_num and adjust as needed to allow basic loop functionality 
		if (this.status.img_num > this.status.total_images ) {
			this.status.img_num = 1;
		}
		if (this.status.img_num < 1) {
			this.status.img_num = this.status.total_images;
		}
	...
	show_current_img() {
		...
		for (let s = 0; s < this.status.total_images; s++) {
			this.component_array.img[s].classList.remove('active');
			this.component_array.img[s].style.display = "none";

			if (this.containers.grid) {
				this.component_array.grid_thb[s].classList.remove('active');
				this.component_array.grid_thb[s].disabled = false;
			} 

			if (this.containers.scroll) {
				this.component_array.scroll_thb[s].classList.remove('active');
				this.component_array.scroll_thb[s].disabled = false;
			}
		}
		...
		this.component_array.img[this.status.img_index].setAttribute('src', this.component_array.img[this.status.img_index].getAttribute('data-src'));
		this.component_array.img[this.status.img_index].classList.add('active');
	...
	toggle_caption() {
		if (this.containers.caption) {
	    	if (this.containers.caption.classList.contains("active")) { 
	...
	--------------------------------- */
	toggle_fscreen() {
		if (this.containers.fscreen_btn) {
			if (this.containers.fscreen_btn.classList.contains("active")) {
				...
				if (this.containers.fscreen.requestFullscreen) {
					this.containers.fscreen.requestFullscreen();
				} else if (this.containers.fscreen.webkitRequestFullscreen) { /* Safari */
					this.containers.fscreen.webkitRequestFullscreen();
				} else if (this.containers.fscreen.msRequestFullscreen) { /* IE11 */
					this.containers.fscreen.msRequestFullscreen();
				}
			}
		}
	}
	...
	--------------------------------- */
	toggle_fwindow() {
		if (this.containers.fwindow_btn) {
	    	if (this.containers.modal_content.style.width === "fit-content") {
				...
				this.containers.modal_content.style.width = "fit-content";
			}
		}
	}
	...
	--------------------------------- */
	download_image() {
		window.open(this.containers.images.querySelector(`.album_${ this.status.album_num }`).querySelector(".active").getAttribute("src"), "_blank", "width=" + (parseInt(window.innerWidth) * 0.2) + ",height=" + (parseInt(window.innerHeight) * .2));
	}
			...
			if (obj === "images" || 
				...
				// Else no template, use default markup
					...
					if (obj === "images") {
						...
						this.markup.grid += `<h4>${ this.status.album_title }</h4>`;
					}
				}
			}
	    });
			...
			if (!this.status.image_file || this.status.image_file === "") {
				...
				this.component_name.forEach((obj) => { /* element, index, array */
					...
					if (obj === "images" || 
						...
						if (obj === "scroll") {
							wrapper_content = "scroll";
						}
						else {
							wrapper_content = obj + "_wrapper_content";
						}
			        	...
			        	// Else no template, use default markup
			        		...
			        		if (obj === "images") {
								this.markup.images += `<img class='img' data-src='${ this.status.image_folder + this.status.image_file }' src=''>`;
							}

							// Build content for each thumbnail in the grid wrapper content
							else if (obj === "grid") {
								this.markup.grid += `<button type='button' class='thb i_${ i }'>${ i }</button>`;
							}

							// Build content for each thumbnail in the pagination scroll
							else if (obj === "scroll") {
								this.markup.scroll += `<button type='button' class='thb i_${ i }'>${ i }</button>`;
							}
						}
	...
	--------------------------------- */
	template_check_2(element) {
		if (this.containers.album_navigation) {
	    	this.markup.album_navigation_menu = '<div class="dropdown_content">';
		}
	    ...
	    this.component_array.dbase.forEach((element) => { /* element, index, array */
			...
			if (this.containers.album_navigation) {
				...
				if (this.template.album_navigation_menu) {
			    	this.markup.album_navigation_menu += this.template.album_navigation_menu(this.status.album_num, this.status.album_title);
			    }
			    // Else no template, use default markup
				else {
					this.markup.album_navigation_menu += `<button class='n_${ this.status.album_num }'>${ this.status.album_title }</button>`;
				}
	...
	thb_listeners(obj, album_num, t) {
		obj.querySelector(`.album_${ album_num }`).addEventListener('click', function (e) {
		    // But only for elements that have an i_t class
		    if (e.target.classList.contains(`i_${ t }`)) {
				//alert(e.target.innerHTML);
				portfolio.slideshow_pause(); 
				portfolio.open_gallery(album_num, t);
			}
		});
	}
	...
	album_listeners(obj, album_num) {
		obj.querySelector(`.n_${ album_num }`).addEventListener('click', function (e) {
		    // But only for elements that have an i_t class
		    //if (e.target.classList.contains(`i_${ t }`)) {
				//alert(e.target.innerHTML);
				portfolio.slideshow_pause(); 
				portfolio.open_gallery(album_num, 1);
			//}
		});
	}
			...
			if (obj === "appearance_btn" ||
				...
				if (this.containers.gallery.querySelector(`.${ obj }`)) {
					this.containers.gallery.querySelector(`.${ obj }`).addEventListener("click", () => {
						...
						if (obj === "appearance_btn" && this.containers.appearance_btn) {
					    	this.toggle_appearance();
						}

						// Toggle Caption
						if (obj === "caption_btn" && this.containers.caption_btn) {
					    	this.toggle_caption();
						}
						...
						// Download button
						if (obj === "download_btn" && this.containers.download_btn) {
					    	this.download_image();
						}

						// Toggle fullscreen
						if (obj === "fscreen_btn" && this.containers.fscreen_btn) {
					    	this.toggle_fscreen();
						}

						// Toggle fullwindow
						if (obj === "fwindow_btn" && this.containers.fwindow_btn) {
					    	this.toggle_fwindow();
						}
						...
						// Go to First image
						if (obj === "first_btn" && this.containers.first_btn) {
						    this.set_img_status(this.status.img_num = 1);
						}
					
						// Go to Last image
						if (obj === "last_btn" && this.containers.last_btn) {
						    this.set_img_status(this.status.img_num = this.status.total_images);
						}

						// Go to Next image
						if (obj === "next_btn" && this.containers.next_btn) {
						    this.set_img_status(this.status.img_num++);
						}

						// Go to Previous image
						if (obj === "previous_btn" && this.containers.previous_btn) {
						    this.set_img_status(this.status.img_num--);
						}
					    ...
					    if (obj === "modal" && this.containers.modal) {  		
					   		...
					   		if (event.target === this.containers.modal) {
								this.close_gallery();
							}
						}
	...
	add_event_listeners() {
		this.component_array.dbase.forEach((element) => { /* element, index, array */
			...
			for (let i = 1; i < this.status.total_images; i++) {
	    		if (this.containers.grid) {
			    	this.thb_listeners(this.containers.grid, this.status.album_num, i);
				}

				if (this.containers.scroll) {
					this.thb_listeners(this.containers.scroll, this.status.album_num, i);
				}
			}
			...
			if (this.containers.album_navigation) {
				this.album_listeners(this.containers.album_navigation, this.status.album_num);	    
			}
	    });



		// Call function to add Event Listeners for slideshow buttons
		this.btn_listeners();



		// Call function to add Event Listeners for keyboard controls
	...
	enable_swipe() {
		...
		function checkDirection() {
			if (touchendX < touchstartX) {
			  	if (portfolio.status.img_num === 1) {
					portfolio.status.img_num = portfolio.status.total_images;
				}
				else { 
					portfolio.status.img_num--;
				}
				portfolio.set_img_status();
			}
